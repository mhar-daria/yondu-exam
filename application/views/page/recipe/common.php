<main class="container fn-main">
	...
	<h1><?=ucwords(preg_replace('/[-]/i', ' ', $title)) ?></h1>
		...
		<?php foreach($records as $recipe): ?>
			...
			<div class="col-md-12 col-xs-12 recipe-container">
				...
				<div class="col-md-8 col-xs-12 details-container">
					...
					<a href="<?=base_url(strtolower($title).'/'.fn_permalink($recipe['recipe_name'])) ?>"><h2 class="title"><?=$recipe['recipe_name'] ?></h2></a>
					<h4 class="category-name"><?=preg_replace('/[-]/i', ' ', $recipe['category_name']) ?></h4>

					<p class="details">
						<?=htmlspecialchars_decode($recipe['recipe_details']) ?>
					</p>
					
					<a href="<?=base_url(strtolower($title).'/'.fn_permalink($recipe['recipe_name'])) ?>" class="read-more">Read More...</a>
				</div>
			</div>
		<?php endforeach; ?>
	</div>
